...
where $\delta_{t-1}$ represents the path probability from the previous step; $A_{ij}$ is the transition probability from state (i.e., the POS tags) $h_i$ to $h_j$; $B_j(w_t)$ shows the emission probability of a word at time step $t$, i.e., $w_t$ at the present state $j$. Another improvement over this algorithm is to store backpointers which stores the previous state leading to the most probable sequence. ~\todo{detail?}
...
% At the first step, Viterbi's algorithm works by setting up a matrix of probabilities, also referred to as a trellis. Each column corresponds to a word (or the observed variable of the HMM) and each row signifies a state in the graph. Each cell is recursively estimated by taking the most probable path that led to the cell. Thus a value of a cell can be formally represented by equation~\ref{eq:viterbi}.

% \begin{equation}~\label{eq:viterbi}
%     \delta_t(j) = \max_{i=1}^{N} \delta_{t-1}(i) A_{ij} B_j(w_t)
% \end{equation}

% where $\delta_{t-1}$ represents the path probability from the previous step; $A_{ij}$ is the transition probability from state (i.e., the POS tags) $h_i$ to $h_j$; $B_j(w_t)$ shows the emission probability of a word at time step $t$, i.e., $w_t$ at the present state $j$.
...
\begin{equation} \label{eq:memm_viterbi}
    %\delta_{i}(j) = \max_{i=1}^{N} \delta_{i-1}(k)P(s_j|s_k,w_t)
...
\section{Dependency Parsing}%~\todo{fix the sections and stuff for this}
...
The process of automatically analyzing dependency structures to an input sentence is referred to as dependency parsing. A dependency tree needs to satisfy the following properties~\citep{kubler2009dependency}:
\begin{itemize}
...
%Since data-driven approaches rely on machine learning approaches, we will only discuss these methods in the rest of the report.
...
Chu-Liu-Edmonds algorithm\citep{chu1965shortest,edmonds1967optimum} often serves as a basis for non-projective parsing approach which determines the maximum spanning tree (MST) of a graph in a greedy and recursive way. We will informally explain the intuition behind the algorithm as follows:
\begin{itemize}[label={--}]
...
I discuss the existing methods for transition based learning from the point of view of projectivity. %Since our objective is to highlight the machine learning approaches, we will look at it from the perspective of the machine learning techniques used.
...
Although SVMs work well in case of parsing, it can be expensive in terms of training and memory requirements. A better workaround for this can be, to use multi layer perceptrons as the classifier~\citep{attardi2009accurate} as perceptrons are faster and require less memory.  
...
Transformation based error driven learning has also been used for information retrieval by \citet{woodley2005applying}\todo{Do I need to write about this?}.
...
%Consider the following example in table~\ref{tab:parser_exeval} which shows that the model trained on the WSJ data parses and produces the output parse indicated . 

% \begin{table}[!htb]
% \centering
% \begin{tabular}{cccccc}
% \begin{tabular}[c]{@{}c@{}}Token\\ Number\end{tabular} & Token & Head & \begin{tabular}[c]{@{}c@{}}Predicted\\ Head\end{tabular} & \begin{tabular}[c]{@{}c@{}}Dependency\\ Label\end{tabular} & \begin{tabular}[c]{@{}c@{}}Predicted\\ Dependency\\ Label\end{tabular} \\ \hline
% 1 & " & 14 & 14 & P & P \\
% 2 & I & 3 & 3 & SBJ & SBJ \\
% 3 & had & 14 & 14 & OBJ & OBJ \\
% 4 & calls & 3 & 3 & OBJ & OBJ \\
% 5 & all & 6 & 6 & NMOD & NMOD \\
% 6 & night & 7 & 4 & AMOD & OBJ \\
% 7 & long & 3 & 4 & TMP & OPRD \\
% 8 & from & 4 & 7 & NMOD & AMOD \\
% 9 & the & 10 & 10 & NMOD & NMOD \\
% 10 & States & 8 & 8 & PMOD & PMOD \\
% 11 & , & 14 & 14 & P & P \\
% 12 & " & 14 & 14 & P & P \\
% 13 & he & 14 & 14 & SBJ & SBJ \\
% 14 & said & 0 & 0 & ROOT & ROOT \\
% 15 & . & 14 & 14 & P & P \\ \hline
% \end{tabular}
% \caption{An example of dependency parse result}\label{tab:parser_exeval}
% \end{table}
...
\hline
\begin{tabular}[c]{@{}l@{}}Token \\ Number\end{tabular} & Token & Head & \begin{tabular}[c]{@{}l@{}}Predicted\\ Head\end{tabular} & \begin{tabular}[c]{@{}l@{}}Dependency\\ Label\end{tabular} & \begin{tabular}[c]{@{}l@{}}Predicted\\ Dependency\\ Label\end{tabular} \\ \hline
1 & " & 3 & 3 & P & P \\
2 & I & 3 & 3 & SBJ & SBJ \\
3 & was & 0 & 0 & ROOT & ROOT \\
4 & woken & 3 & 3 & VC & VC \\
5 & up & 4 & 4 & PRT & PRT \\
6 & every & 7 & 7 & NMOD & NMOD \\
7 & hour & 4 & 4 & OBJ & TMP \\
8 & -- & 7 & 4 & P & P \\
9 & 1:30 & 15 & 4 & NMOD & TMP \\
10 & , & 15 & 9 & P & P \\
11 & 2:30 & 15 & 9 & NMOD & APPO \\
12 & , & 15 & 9 & P & P \\
13 & 3:30 & 15 & 9 & NMOD & APPO \\
14 & , & 15 & 9 & P & P \\
15 & 4:30 & 7 & 9 & APPO & APPO \\
16 & . & 3 & 3 & P & P \\ \hline
\end{tabular}
\caption{An example of dependency parse result}\label{tab:parser_exeval}
\end{table}
...
% 3   16    0   50.00  43.75     8       7          16    0 0 0 0

Table~\ref{tab:parser_exeval} shows an example of the dependency parse output given by a parse model trained on the WSJ data. 
